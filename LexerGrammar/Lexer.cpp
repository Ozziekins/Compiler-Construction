...
Token Lexer::readLiteral() {
    ...
    if (isdigit(currentChar)) {
        ...
        if (dot_encountered) {
            Token token = Token(Token::TOKEN_REAL_LITERAL, tokenValue, currentLine,
                                currentPosOnLine - tokenValue.length());
            tokenList.push_back(token);
            finalToken = token;
        } else {
            Token token = Token(Token::TOKEN_INT_LITERAL, tokenValue, currentLine,
                                currentPosOnLine - tokenValue.length());
            tokenList.push_back(token);
...
Token Lexer::readIdentifier() {
    ...
    if (specialCharMappings.count(std::string() + unknownTokenValue) != 0) {
        Token token = Token(specialCharMappings[std::string() + unknownTokenValue], unknownTokenValue, currentLine,
                            currentPosOnLine - unknownTokenValue.length());
        tokenList.push_back(token);
        return token;
    } else {
        Token token = Token(Token::TOKEN_IDENTIFIER, unknownTokenValue, currentLine,
                            currentPosOnLine - unknownTokenValue.length());
        tokenList.push_back(token);
        return token;
    }
